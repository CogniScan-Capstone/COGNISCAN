    ...
    <main className="min-h-screen bg-surface px-5 py-12 text-on-surface">
      <section className="mx-auto max-w-[580px] rounded-[24px] bg-white px-8 py-12 shadow-[0_24px_45px_-32px_rgba(27,28,26,0.36)] sm:px-10">
        ...
        <div className="space-y-4">
          {initialPsychologists.map((psy) => {
            const isSelected = selectedId === psy.id;
            return (
              <article
                key={psy.id}
              ...
              >
                ...
                <button
                  type="button"
                  onClick={() => setSelectedId(isSelected ? null : psy.id)}
        ...
        <Link
          ...
          className="mt-8 inline-flex h-14 w-full items-center justify-center gap-3 rounded-full bg-primary-container px-8 text-[18px] font-medium text-white shadow-[0_18px_28px_-20px_rgba(65,87,62,0.75)] transition hover:-translate-y-0.5 hover:bg-[#4d734d]"
        >
          Submit
